...
x %>% 
  ...
  select(path_cln, true_author, work)
  

# for each author: subset the table, 
# extract the bdi results & reshape
# then make a plot (colours -- works, highlight the author)
# store plot for each author
```
...
```{r}
plt <- compile_res("03_tests/authors_vs_all/runs/", ptrn = "^Baudeau")
...
```{r}
plt %>% 
    mutate(group = str_remove_all(group, "03_tests/authors_vs_all/runs//|\\.csv")) %>% 
    ggplot(aes(x = bdi, group = run)) + 
    geom_density(alpha = 0.15, fill = "darkgreen", colour = "black") + 
    geom_vline(xintercept=0, lty = 2, colour = "black") + 
    facet_wrap(~group, ncol=1) + 
    theme(legend.position = "None") + 
    labs(title = paste0("Baudeau-1 vs various authors (2k chunks, 200 MFW)"))
```
