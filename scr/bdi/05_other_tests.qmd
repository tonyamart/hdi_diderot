...
plt %>% 
  ...
  mutate(group = ifelse(group == "pencil_vs_dHolbach", "pencil_vs_d'Holbach", group)) %>% 
  mutate(group = ifelse(group == "pencil_vs_Rivière", "pencil_vs_La Rivière", group)) %>%
  ...
  facet_wrap(~group, ncol=1) + 
  theme(legend.position = "None") + 
  labs(title = "Pencil vs selected authors",
       y = "") + 
...
fig_3b <- plt %>% 
  mutate(group = str_remove_all(group, "05_tests/tests//|\\.csv")) %>% 
  filter(str_detect(group, "Condorcet|Diderot|dHolbach|Deleyre|Chastellux|Marmontel|Saint-Lambert|Raynal")) %>% 
  mutate(gr = ifelse(str_detect(group, "Diderot"), "d", "no")) %>% 
  ...
  scale_fill_manual(values = met.brewer("Kandinsky")[2:4]) + 
  #geom_vline(xintercept=0.1, lty = 2, colour = "black") + 
  facet_wrap(~factor(group, levels = c("ink_vs_Condorcet",
                                       "ink_vs_Diderot",
                                       "ink_vs_d'Holbach",
                                       "ink_vs_Deleyre",
                                       "ink_vs_Chastellux",
                                       "ink_vs_Marmontel",
                                       "ink_vs_Saint-Lambert",
                                       "ink_vs_Raynal")), 
             ncol=1) + 
  theme(legend.position = "None") + 
  labs(title = "   Mélanges (ink) vs selected authors (top BDI results)",
       y = "", x = "BDI") + 
  theme(strip.text = element_text(size = 12),
...
fig_4b <- plt %>% 
  mutate(group = str_remove_all(group, "05_tests/tests//|\\.csv")) %>% 
  filter(str_detect(group, "Diderot|Condorcet|Deleyre|Chastellux|Guibert|Marmontel|Morellet|Saint-Lambert")) %>% 
  mutate(gr = ifelse(str_detect(group, "Diderot"), "d", "no")) %>% 
  ...
  scale_fill_manual(values = met.brewer("Kandinsky")[2:4]) + 
  #geom_vline(xintercept=0.1, lty = 2, colour = "black") + 
  facet_wrap(~factor(group, levels = c("sauvage_vs_Diderot",
                                       "sauvage_vs_Condorcet",
                                       "sauvage_vs_Deleyre",
                                       "sauvage_vs_Chastellux",
                                       "sauvage_vs_Guibert",
                                       "sauvage_vs_Marmontel",
                                       "sauvage_vs_Morellet",
                                       "sauvage_vs_Saint-Lambert")), 
             ncol=1) + 
  theme(legend.position = "None") + 
  labs(title = "   'Sauvage' fragment vs selected authors (top BDI results)",
       y = "", x = "BDI") + 
  theme(strip.text = element_text(size = 12),
        plot.title = element_text(size = 16),
        axis.text.y = element_blank())
