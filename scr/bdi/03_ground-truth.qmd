...
fig_1c <- new_data %>% 
  mutate(group = str_remove_all(group, "03_tests/fp1/2000_words/mfw200//|\\.csv")) %>% 
  filter(str_detect(group, "Diderot|dHolbach|Condorcet|Deleyre|Morellet")) %>% 
  mutate(gr = ifelse(str_detect(group, "Diderot"), "d", "no")) %>% 
  ...
  # reorder plots according to mean % above 0  
  facet_wrap(~factor(group, levels = c("fp1_vs_Diderot", 
                                       "fp1_vs_Condorcet",
                                       "fp1_vs_d'Holbach",
                                       "fp1_vs_Deleyre",
                                       "fp1_vs_Morellet")),
